...
function Render()
  ...
  cdbCanvas:Clear()
    ...
    if (boxc.x-iw/2)%10~=0 or (boxc.y+ih/2)%10~=0 then
      ...
      if (boxc.x-iw/2)%10~=0 and (boxc.x-iw/2)%10>=5 then
        tempx = boxc.x-iw/2 + (10 - (boxc.x-iw/2)%10 )  -- if remdnder of boxc.x-wh/2 with 10 is greater then 5 then we will take upper bound     
      else
        tempx = boxc.x-iw/2  - (boxc.x-iw/2)%10 --else we will take lower bound
      end
      -- (boxy.c+ih/2) is not multiple of 10

      if (boxc.y+ih/2)%10~=0 and (boxc.y+ih/2)%10>=5 then
        tempy = boxc.y+ih/2 + (10 - (boxc.y+ih/2)%10 )
      else
