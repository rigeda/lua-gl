...
function cnv:motion_cb(x, y, r)
  ...
  if move then
	-- move the box here
	cdbCanvas:PutImageRect(img, boxc.x-10, cdbCanvas:UpdateYAxis(boxc.y-10),0,0,0,0)
	cdbCanvas:GetImage(img, x-move[1], cdbCanvas:UpdateYAxis(y-move[2]))
	boxc.x = x-move[1]+10	-- move[1] and move[2] contain the offset of the mouse pointer from the lower left corner of the box
	boxc.y = y-move[2]+10	-- So set the new center coordinates on the boxc array which would be used by the rendering function to draw the box
	Render()
  end
end
...
function Render()
	...
	if not img then
		...
		img = cdbCanvas:CreateImage(20,20)
		print("Create initial box")
		cdbCanvas:GetImage(img, boxc.x-10, cdbCanvas:UpdateYAxis(boxc.y-10))
		-- Draw the box
		...
		cdbCanvas:Sector(boxc.x-10,cdbCanvas:UpdateYAxis(boxc.y-10),10,10,0,360)
		cdbCanvas:SetForeground(cd.EncodeColor(0, 255, 0))
		cdbCanvas:Sector(boxc.x-10,cdbCanvas:UpdateYAxis(boxc.y+10),10,10,0,360)
	else
		print("PutImageRect")
		cdbCanvas:PutImageRect(boxImg, boxc.x-10, cdbCanvas:UpdateYAxis(boxc.y+10),0,0,0,0)	-- Put the box image in the new coordinates
	end
	if not boxImg then
		-- Executing 1st time so get the image of the drawn box
		boxImg = cdbCanvas:CreateImage(20,20)
		cdbCanvas:GetImage(boxImg, boxc.x-10, cdbCanvas:UpdateYAxis(boxc.y+10))	-- y + 10 because that is the lower left corner of the box
	end
	if doline then
		cdbCanvas:SetForeground(cd.EncodeColor(255, 0, 0))
...
function cnv:button_cb(button,pressed,x,y,status)
	if button == iup.BUTTON1 then
		if pressed == 1 then
			...
			if x <= boxc.x + 10 and x >= boxc.x-10 and y <= boxc.y+10 and y >= boxc.y-10 then
				print("SET MOVE")
				move = {x-boxc.x+10,y-boxc.y+10}
			end
		else
